...
# def top_partition_library(landscape, full_library, percent = 10, sort_by = 'count'):
#     ''' returns the top percent of a landscape as a library
#     '''
#     top_partition = []

#     beg = [0 for i in range(landscape.N)]
#     energies = [landscape.get_Energy(i) for i in full_library]

#     seq_nrg_df = pd.DataFrame({'Seq' : full_library, 'Energy' : energies})
#     seq_nerg = seq_nrg_df.sort_values(by = 'Energy')

#     top_by_count = []
#     top_by_value = []

#     top_count = math.floor(percent/100*len(full_library))
#     top_value_cutoff = seq_nrg*(1 - percent/100)
...
import time
n_list = [2,3,4,5,6]             #sequence length
library_size = 100

for n in n_list:
    beg = time.clock()
    landscape = NK_Landscape.NKLandscape(n,1, savespace = False, epi_dist = 'gamma')
    ...
    tbc = seq_nrg_df.nlargest(top_count, 'Energy')
    tbv = seq_nrg_df[seq_nrg_df.Energy > top_value_cutoff]

    total_time = time.clock()-beg
